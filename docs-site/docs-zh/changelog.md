...
- 新增 JSON 输出模式（`-json` 参数）—— 为 AI 代理和自动化输出结构化 JSON 结果，JSON 模式下自动抑制进度条
- 新增 SSH 端口转发（`-L` 本地转发、`-R` 远程转发）及 `Forwarder` SDK API —— 通过 SSH 隧道传输 TCP 连接
- 新增 ssh-agent 认证与转发（`-A` 参数，自动检测）—— 无密码/密钥时自动使用本地 ssh-agent
- 新增键盘交互式认证回退，兼容 PAM/Cisco 服务器
- 新增 `ExecCapture` SDK 方法 —— 以编程方式捕获命令输出（非流式）
- 从仓库中移除 docker-test
- 多项代码清理与重构
