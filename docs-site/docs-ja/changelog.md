...
- JSON 出力モード（`-json` フラグ）を追加 — AI エージェントと自動化のための構造化 JSON 出力、JSON モードでプログレスバーを抑制
- SSH ポート転送（`-L` ローカル、`-R` リモート）と `Forwarder` SDK API を追加 — SSH 経由で TCP 接続をトンネリング
- ssh-agent 認証と転送（`-A` フラグ、自動検出）を追加 — パスワード/鍵なしで自動的にローカル ssh-agent を使用
