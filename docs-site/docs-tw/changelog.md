...
- 新增 JSON 輸出模式（`-json` 參數）—— 為 AI 代理和自動化輸出結構化 JSON 結果，JSON 模式下自動抑制進度條
- 新增 SSH 連接埠轉送（`-L` 本地轉送、`-R` 遠端轉送）及 `Forwarder` SDK API —— 透過 SSH 隧道傳輸 TCP 連線
- 新增 ssh-agent 認證與轉送（`-A` 參數，自動檢測）—— 無密碼/金鑰時自動使用本地 ssh-agent
- 新增鍵盤互動式認證回退，相容 PAM/Cisco 伺服器
- 新增 `ExecCapture` SDK 方法 —— 以程式化方式擷取命令輸出（非串流）
- 從儲存庫中移除 docker-test
- 多項程式碼清理與重構
